...
class AwsEc2DeleteSecurityGroupV1
  ...
  def execute() 
    ...
    if @parameters['group_name'].strip!=""
      inputs[:group_name] = @parameters['group_name']
    end

    if @parameters['group_id'].strip!=""
      inputs[:group_id] = @parameters['group_id']
    end

    resp = @ec2.delete_security_group(inputs)

    message="Security Group #{@parameters['group_name']}#{@parameters['group_id']} deleted successfully."
