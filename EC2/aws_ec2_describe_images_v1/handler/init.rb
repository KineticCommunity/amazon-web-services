...
class AwsEc2DescribeImagesV1
  ...
  def execute() 
    ...
    return_xml = "<results>\n"
    return_xml += "<result name='images_info_string'>#{escape(resp.images.inspect)}</result>\n"
    return_xml += "<result name='images_images_id_string'>#{escape(images_id_array.to_s)}</result>\n"
