...
class AwsEc2CreateKeyPairV1
  ...
  def execute() 
    ...
    puts "Key Name: #{resp.key_name}, Key Fingerprint: #{resp.key_fingerprint}, Key Material: #{resp.key_material}" if @enable_debug_logging

    <<-RESULTS
    <results>
      <result name="key_name">#{escape(resp.key_name)}</result>
      <result name="key_fingerprint">#{escape(resp.key_fingerprint)}</result>
      <result name="key_material">#{escape(resp.key_material)}</result>
    </results>
    RESULTS
  end
